            ...
            try:
                ...
                def train_knn():
                    ...
                    if len(known_faces) > 0:
                        knn = KNeighborsClassifier(n_neighbors=min(5, len(known_faces)))
                        X = np.array(known_faces)
                        y = np.array(known_names)
                        knn.fit(X, y)
                
                def recognize_face(face_image):
                    encoding = face_image.flatten()  
                    if len(known_faces) > 0 and knn is not None:
                        distances, indices = knn.kneighbors([encoding])
                        if distances[0][0] < 3000:  
                            return known_names[indices[0][0]]
                    return "Unknown"
                ...
                while True:
                    ...
                    for (x, y, w, h) in faces:
                        ...
                        # Placeholder for KNN prediction
                        knn_name = recognize_face(gray[y : y + h, x : x + w])
